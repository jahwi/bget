...
if "!missing_curl!"=="yes" (
	...
	if "!errorlevel!"=="1" (
		...
		call :download -js "https://archive.org/download/batchget_gmail_Curl/curl.exe#%cd%\curl\curl.exe https://archive.org/download/batchget_gmail_Curl/curl-ca-bundle.crt#%cd%\curl\curl-ca-bundle.crt https://archive.org/download/batchget_gmail_Curl/libcurl.dll#%cd%\curl\libcurl.dll"
...
if /i "%1"=="-bits" (
	set /a rnd=%random%
	set bits_string=%~2
	bitsadmin /transfer Bget!rnd! /download /priority HIGH !bits_string:#= !
	set rnd=
	set bits_string=
	exit /b
)
...
if /i "%1"=="-js" (
	...
	if exist "bin\download.js" (
		for %%x in (%~2) do (
			set jsstring=%%x
			cscript //NoLogo //e:Jscript bin\download.js !jsstring:#= !
			set jsstring=
		)
		exit /b
	)
...
if /i "%1"=="-vbs" (
	...
	if exist "bin\download.vbs" (
		for %%x in (%~2) do (
			set vbsstring=%%x
			cscript //NoLogo //e:VBscript bin\download.vbs !vbsstring:#= !
			set vbsstring=
		)
		exit /b
	)
